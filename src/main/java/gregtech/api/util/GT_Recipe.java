...
//		checkCellBalance();
    ...
    public boolean isRecipeInputEqual(boolean aDecreaseStacksizeBySuccess, boolean aDontCheckStackSizes, FluidStack[] aFluidInputs, ItemStack... aInputs) {
        ...
        boolean[] blacklistMattered = {false};
        for (ItemStack tStack : mInputs) {
            if (tStack != null) {
                amt = tStack.stackSize;
                boolean temp = true;
                ItemStack unified_tStack = GT_OreDictUnificator.get(tStack);
                for (ItemStack aStack : aInputs) {
                    ItemStack unified_aStack = GT_OreDictUnificator.getStackAndWhetherBlacklistHadAnyEffect(aStack, blacklistMattered);
                    if ((GT_Utility.areStacksEqual(unified_aStack, unified_tStack, true)
                            || (blacklistMattered[0] && GT_Utility.areStacksEqual(GT_OreDictUnificator.get(false, aStack), unified_tStack, true)))) {
